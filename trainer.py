...
from utils.samplers import DownSampler, ProbabilisticAugMix, AugMixWrapper
...
class Trainer:
    ...
    def test_time(self, writer_dir):
        # Ensure all CUDA kernels are complete before starting timing
        ...
        log_line = f"[{datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] Test Inference Time: {elapsed_time:.2f} seconds\n"    
        print(log_line.strip())
        log_path = Path(self.cfg.output_dir) / "test_time_log.txt"
    ...
    def build_data_loader(self):
        ...
        if cfg.tte:
            ...
            elif cfg.tte_mode == "multiscale":
                scales = [0, 32, 64]  # 对应 resize 到 resolution、resolution+32、resolution+64
                transform_test = transforms.Lambda(lambda image: torch.stack([
                    transforms.Compose([
                        transforms.Resize(resolution + offset),
                        transforms.CenterCrop(resolution),
                        #transforms.RandomHorizontalFlip(p=1.0 if flip else 0.0),  # 控制是否翻转
                        transforms.ToTensor(),
                        transforms.Normalize(mean, std),
                    ])(image)
                    for offset in scales        
                ]))  # 输出为 [6, C, H, W]  
            elif cfg.tte_mode == "multiscaleflip":
                scales = [0, 32, 64]  # 对应 resize 到 resolution、resolution+32、resolution+64
                transform_test = transforms.Lambda(lambda image: torch.stack([
                    transforms.Compose([
                        transforms.Resize(resolution + offset),
            ...
            elif cfg.tte_mode == "fixcrop_flip":
                crop_offsets = [ # Relative crop positions (as a fraction of the resized image)
                    (0.0, 0.0),  # top-left
                    (0.5, 0.5),  # center
                    (1.0, 1.0),]  # bottom-right            
                def fixed_crop(img, offset_x, offset_y, crop_size):
                    w, h = img.size
                    th, tw = crop_size, crop_size
                    i = int((h - th) * offset_y)
                    j = int((w - tw) * offset_x)
                    return img.crop((j, i, j + tw, i + th))        
                transform_test = transforms.Lambda(lambda img: torch.stack([
                    transforms.Normalize(mean, std)(
                        transforms.ToTensor()(fixed_crop(
                            transforms.Resize(resolution + expand)(img), ox, oy, resolution))
                    ) if not flip else
                    transforms.Normalize(mean, std)(
                        transforms.ToTensor()(transforms.functional.hflip(
                            fixed_crop(transforms.Resize(resolution + expand)(img), ox, oy, resolution))))
                    for (ox, oy) in crop_offsets
                    for flip in [False, True]
                ]))  # Output shape: [6, C, H, W]                  
        else:
            transform_test = transforms.Compose([
        ...
        self.cls_num_list = train_dataset.cls_num_list #raw_dataset.cls_num_list #
        self.classnames = train_dataset.classnames  #raw_dataset.classnames #
        ...
        self.test_loader = DataLoader(test_dataset,
            batch_size=32, shuffle=False,  #64
            num_workers=cfg.num_workers, pin_memory=True)
    ...
    def build_model(self):
        ...
        cfg.cls_num_list = self.cls_num_list
    ...
    def build_optimizer(self):
        ...
        self.scaler = GradScaler(device='cuda') if cfg.prec == "amp" else None
    ...
    def build_criterion(self):
        ...
        if cfg.loss_type == "CE":
            ...
            self.criterion = LDAMLoss(cfg, cls_num_list=cls_num_list, s=cfg.scale)
        elif cfg.loss_type == "CB": # https://arxiv.org/abs/1901.05555
            self.criterion = ClassBalancedLoss(cfg,cls_num_list=cls_num_list)
        elif cfg.loss_type == "GRW": # https://arxiv.org/abs/2103.16370
            self.criterion = GeneralizedReweightLoss(cfg, cls_num_list=cls_num_list)
        elif cfg.loss_type == "BS": # https://arxiv.org/abs/2007.10740
            self.criterion == BalancedSoftmaxLoss(cfg, cls_num_list=cls_num_list)
        elif cfg.loss_type == "LA": # https://arxiv.org/abs/2007.07314
            self.criterion = LogitAdjustedLoss(cfg, cls_num_list=cls_num_list)
        elif cfg.loss_type == "LADE": # https://arxiv.org/abs/2012.00321
            self.criterion = LADELoss(cfg, cls_num_list=cls_num_list)
        elif cfg.loss_type == "TKLA": #  
            self.criterion = TopKLogitAdjustedLoss(cfg, cls_num_list)
        elif cfg.loss_type == "GCL": #  
            self.criterion = GCLLoss(cfg, cls_num_list)

    def mixup_loss_fn(self, logits, labels1, labels2, lam, criterion):
        #per sample loss
        loss1 = criterion(logits, labels1, reduction='none')  # shape: [B]
        loss2 = criterion(logits, labels2, reduction='none')  # shape: [B]        
        loss = lam * loss1 + (1 - lam) * loss2  # shape: [B]        
        return loss.mean()  
    ...
    def train(self):
        ...
        for epoch_idx in range(num_epochs):
            ...
            if hasattr(self.criterion, "_hook_before_epoch"):   
                self.criterion._hook_before_epoch(epoch_idx)            
            #if hasattr(self.criterion, "_update_cost_embed"):   
            #    self.criterion._update_cost_embed(self.model.head.weight.data)
            for batch_idx, batch in enumerate(self.train_loader):
                ...
                if cfg.prec == "amp":
                    with autocast(device_type='cuda'):
                        ...
                        #loss = self.mixup_loss_fn(output, labels1, labels2, lam, self.criterion)
            ...
            if acc > max_acc :
                ...
                self.save_model(cfg.output_dir,'epoch-best.pth.tar')
            print( f"* best_acc: {max_acc:.1f}%, best epoch: {best_epoch+1}")
        
        best_epoch = best_epoch+1        
        print(f"Finish training. Bset epoch: {best_epoch}")
        ...
        # save model
        self.save_model(cfg.output_dir, 'epoch-last.pth.tar')
