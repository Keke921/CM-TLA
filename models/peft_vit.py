...
class ViT_Tuner(nn.Module):
    ...
    def __init__(self, cfg, vit_model, num_classes):
        ...
        use_cwadaptformer = cfg.cwadaptformer
        use_lora = cfg.lora
        ...
        cls_num_list = torch.Tensor(cfg.cls_num_list).to('cuda')  
        if partial is None:
        ...
        if (use_adapter or use_adaptformer or use_cwadaptformer or use_lora or use_lora_mlp) and (adapter_dim is None):
        ...
        if use_cwadaptformer:
            cwadaptformer_list = nn.ModuleList([
                *[None] * (_start),
                *[ClassWiseAdaptFormer(in_dim=emb_dim, bottle_dim=adapter_dim, cls_num_list = cls_num_list, dtype=dtype) for _ in range(_start, _end)],                
                *[None] * (n_layers - _end)
            ])
        else:
            cwadaptformer_list = nn.ModuleList([None] * n_layers)
        ...
        self.cwadaptformer_list = cwadaptformer_list       
...
class Peft_ViT(nn.Module):
    ...
    def forward(self, x, target = None, tuner=None, head=None):
        x = x.to(self.dtype)
        ...
        for i in range(n_layers):
            ...
            if tuner is not None:
                ...
                cwadaptformer = tuner.cwadaptformer_list[i]
                ...
                vpt = adapter = adaptformer = cwadaptformer = lora = lora_mlp = ssf_attn = ssf_mlp = ssf_ln = masked_linear = None
            ...
            if cwadaptformer is not None:
                x = x + cwadaptformer(identity, target)
