...
class PeftModelFromCLIP(nn.Module):
    ...
    def prepare_for_inference(self):
        """
        Call this before evaluation to cache FiLM parameters for each ClassWiseAdaptFormer.
        """
        if hasattr(self.tuner, "prepare_for_inference"):
            self.tuner.prepare_for_inference()
    
    def forward(self, image, target = None, use_tuner=True, return_feature=False):
        tuner = self.tuner if use_tuner else None
        head = self.head if not return_feature else None
        return self.image_encoder(image, target, tuner, head)
